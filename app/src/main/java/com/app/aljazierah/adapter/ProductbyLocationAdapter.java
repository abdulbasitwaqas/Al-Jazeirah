...
public class ProductbyLocationAdapter extends RecyclerView.Adapter<ProductbyLocationAdapter.MyViewHolder> {

    public List<ProductByLocation> productList;
    ...
    public class MyViewHolder extends RecyclerView.ViewHolder {
        ...
        private ImageView addproduct_button, minusproduct_button, oneStarIV, twoStarIV, threeStarIV, fourStarIV, fiveStarIV;
        ...
        private ImageView productsImage,imageWhish,outOfStockNotification;


        public MyViewHolder(View view) {
            super(view);
            mainRL = view.findViewById(R.id.cardView);
...
//            productsImage = view.findViewById(R.id.imageProductThumb);
    ...
    public ProductbyLocationAdapter(Context context, ArrayList<ProductByLocation> productList, FragmentHome fragmentHome, Activity activity) {
        this.productList = productList;
        this.mContext = context;
        this.activity = activity;
        this.isArabic = AppController.setLocale();
        this.mProductsFragment = fragmentHome;
    }


    public ProductbyLocationAdapter(Context context, ArrayList<ProductByLocation> productList, Activity activity) {
        this.productList = productList;
        this.mContext = context;
        this.activity = activity;
        this.isArabic = AppController.setLocale();
...
//        Log.d("**productRating","rating:  "+productList.get(position).getProd_rating());
//        Log.d("**productCount","rating:  "+productList.get(position).getCount());
...
//                CleverTapAPI.getDefaultInstance(mContext).pushEvent("Added to Cart:",eventValue);
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.LOLLIPOP){
            holder.mainRL.setClipToOutline(true);
            holder.productsImage.setClipToOutline(true);
        }
...
//                CleverTapAPI.getDefaultInstance(mContext).pushEvent("Added to Cart:",eventValue);
    ...
    private void ArabicData(MyViewHolder holder, int position) {
        ...
        //          setTypeface(Typeface.createFromAsset(mContext.getAssets(), "fonts/poppins_bold.ttf"), Typeface.NORMAL);
...
//                    setTypeface(Typeface.createFromAsset(mContext.getAssets(), "fonts/neo_sans_bold.ttf"), Typeface.BOLD);
    ...
    private void EnglishData(MyViewHolder holder, int position) {
        // holder.si_home_Products_tvTitle.setTypeface(Typeface.createFromAsset(mContext.getAssets(), "fonts/poppins_bold.ttf"), Typeface.NORMAL);
    ...
    private void stockNotification(ProductByLocation productByLocation){
        User user = DatabaseManager.getInstance().getFirstOfClass(User.class);
        String action = "1";
        String removeaction = "9";
        showProgress(true);
        Log.d("**reqBody", new Gson().toJson(new AddToWishStockAlertRequest(user.userId,productByLocation.getId(),"stock_alert",action)));
  ...
  private void addToWishList(ProductByLocation productByLocation,int position){
        ...
        showProgress(true);
      Log.d("**favrequestbodyAd", new Gson().toJson(new AddToWishStockAlertRequest(user.userId,productByLocation.getId(),"wish_list",action)));
      APIManager.getInstance().addTowishListStockAlert(new APIManager.Callback() {
            @Override
            public void onResult(boolean z, String response) {
                showProgress(false);
